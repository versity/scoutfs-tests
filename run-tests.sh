...
test -n "$T_DATA_DEVICE" || die "must specify -d fs data device"
test -e "$T_DATA_DEVICE" || die "fs datadevice -d '$T_DATA_DEVICE' doesn't exist"
test -n "$T_META_DEVICE" || die "must specify -z fs meta device"
test -e "$T_META_DEVICE" || die "fs metadevice -z '$T_META_DEVICE' doesn't exist"

test -n "$T_EX_META_DEV" || die "must specify -E extra meta device"
test -e "$T_EX_META_DEV" || die "fs device -E '$T_EX_META_DEV' doesn't exist"
test -n "$T_EX_DATA_DEV" || die "must specify -e extra data device"
test -e "$T_EX_DATA_DEV" || die "fs device -e '$T_EX_DATA_DEV' doesn't exist"
